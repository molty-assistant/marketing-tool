...
function Chips({ items }: { items: string[] }) {
  ...
  return (
    <div className="flex flex-wrap gap-1.5">
      {items.map((item, i) => (
        <span key={`${item}-${i}`} className="text-xs bg-card border border-border text-muted-foreground px-2.5 py-1 rounded-full">
          {item}
        </span>
      ))}
    </div>
...
export default function FoundationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
        <div>
          <div className="flex items-center gap-3 flex-wrap">
            <h1 className="text-2xl font-bold text-foreground">🧱 Foundation Layer</h1>
          ...
          <p className="text-muted-foreground">Brand voice, positioning angles &amp; competitive intel for {plan.config.app_name}</p>
        </div>
      </div>

      {/* Jump-links */}
      <nav className="sticky top-0 z-10 bg-muted/80 backdrop-blur-md border-b border-border -mx-4 px-4 py-2.5 mb-8 flex items-center gap-3 overflow-x-auto">
        {[
          ...
          <a
            key={s.id}
            href={`#${s.id}`}
            className="text-sm text-muted-foreground hover:text-foreground whitespace-nowrap px-3 py-1.5 rounded-lg hover:bg-card transition-colors"
          >
            {s.label}
          </a>
        ))}
      ...
      <section id="brand-voice" className="bg-card border border-border rounded-2xl p-6 mb-8 scroll-mt-16">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            <h2 className="text-lg font-semibold text-foreground">🎙️ Brand Voice</h2>
            <p className="text-sm text-muted-foreground">A usable voice profile for copy &amp; creative.</p>
          </div>
        ...
        {!brandVoice ? (
          <div className="mt-5 text-muted-foreground text-sm">Not generated yet.</div>
        ) : (
          <div className="mt-5 space-y-5">
            {/* Summary */}
            <div className="bg-card border border-border rounded-2xl p-4">
              <div className="text-sm font-semibold text-foreground mb-2">Voice summary</div>
              <div className="text-muted-foreground text-sm leading-relaxed">{brandVoice.voiceSummary}</div>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              {/* Traits */}
              <div className="bg-card border border-border rounded-2xl p-4">
                <div className="text-sm font-semibold text-foreground mb-3">Personality traits</div>
                <div className="space-y-3">
                  {brandVoice.personalityTraits?.map((t, i) => (
                    <div key={`${t.trait}-${i}`} className="border border-border rounded-xl p-3">
                      <div className="text-sm text-foreground font-semibold">{t.trait}</div>
                      <div className="text-xs text-muted-foreground mt-1">{t.description}</div>
                      <div className="text-xs text-muted-foreground mt-2 bg-muted border border-border rounded-lg p-2 italic">&ldquo;{t.example}&rdquo;</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-4">
                {/* Vocabulary */}
                <div className="bg-card border border-border rounded-2xl p-4">
                  <div className="text-sm font-semibold text-foreground mb-3">Vocabulary guide</div>
                  <div className="space-y-3">
                    <div><div className="text-xs text-muted-foreground mb-2">✅ Words to use</div><Chips items={brandVoice.vocabularyGuide?.wordsToUse || []} /></div>
                    <div><div className="text-xs text-muted-foreground mb-2">❌ Words to avoid</div><Chips items={brandVoice.vocabularyGuide?.wordsToAvoid || []} /></div>
                    <div><div className="text-xs text-muted-foreground mb-2">✅ Phrases to use</div><Chips items={brandVoice.vocabularyGuide?.phrasesToUse || []} /></div>
                    <div><div className="text-xs text-muted-foreground mb-2">❌ Phrases to avoid</div><Chips items={brandVoice.vocabularyGuide?.phrasesToAvoid || []} /></div>
                  </div>
                </div>

                {/* Tone */}
                <div className="bg-card border border-border rounded-2xl p-4">
                  <div className="text-sm font-semibold text-foreground mb-3">Tone spectrum</div>
      ...
      <section id="positioning" className="bg-card border border-border rounded-2xl p-6 mb-8 scroll-mt-16">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            <h2 className="text-lg font-semibold text-foreground">🎯 Positioning Angles</h2>
            <p className="text-sm text-muted-foreground">3–5 distinct ways to frame the product.</p>
          </div>
        ...
        {!positioning ? (
          ...
          <div className="mt-5 space-y-4">
            <div className="bg-indigo-950/25 border border-indigo-800/40 rounded-2xl p-4 text-sm text-muted-foreground">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {positioning.angles?.map((a) => {
                ...
                return (
                  <div key={a.name} className="bg-card border border-border rounded-2xl overflow-hidden">
                    <button onClick={() => setExpandedAngle(open ? null : a.name)} className="w-full text-left p-4 hover:bg-muted/50 transition-colors">
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <div className="text-foreground font-semibold">{a.name}</div>
                          <div className="text-muted-foreground text-sm mt-1">{a.hook}</div>
                        </div>
                    ...
                    {open && (
                      <div className="p-4 pt-0 space-y-3">
                        <div><div className="text-xs text-muted-foreground mb-1">Why it works</div><div className="text-sm text-muted-foreground">{a.psychology}</div></div>
                        <div><div className="text-xs text-muted-foreground mb-1">Headline directions</div>
                          <ul className="list-disc pl-5 text-sm text-muted-foreground space-y-1">
                            {a.headlineDirections?.map((h, i) => <li key={i}>{h}</li>)}
                          </ul>
                        </div>
                        <div><div className="text-xs text-muted-foreground mb-1">Best for</div><div className="text-sm text-muted-foreground">{a.bestFor}</div></div>
                      </div>
                    )}
                  </div>
                );
            ...
            <div className="bg-card border border-border rounded-2xl p-4">
              <div className="text-sm font-semibold text-foreground mb-2">Anti-positioning</div>
              <div className="text-xs text-muted-foreground mb-2">What we are NOT</div>
              <Chips items={positioning.antiPositioning?.whatWeAreNot || []} />
              <div className="text-xs text-muted-foreground mt-4 mb-1">Why it matters</div>
              <div className="text-sm text-muted-foreground">{positioning.antiPositioning?.whyItMatters}</div>
            </div>
          </div>
        )}
      </section>

      <section id="competitive" className="bg-card border border-border rounded-2xl p-6 mb-10 scroll-mt-16">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            <h2 className="text-lg font-semibold text-foreground">⚔️ Competitive Analysis</h2>
            <p className="text-sm text-muted-foreground">Competitors, gaps, opportunities &amp; messaging.</p>
          </div>
        ...
        {!competitive ? (
          ...
          <div className="mt-5 space-y-6">
            ...
            <div className="overflow-x-auto border border-border rounded-2xl">
              <table className="min-w-[900px] w-full text-sm">
                <thead className="bg-card text-muted-foreground">
                  <tr>
                    <th className="text-left p-3 border-b border-border">Competitor</th>
                    <th className="text-left p-3 border-b border-border">Positioning</th>
                    <th className="text-left p-3 border-b border-border">Pricing</th>
                    <th className="text-left p-3 border-b border-border">Strengths</th>
                    <th className="text-left p-3 border-b border-border">Weaknesses</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    <tr key={c.url || c.name} className="align-top">
                      <td className="p-3 border-b border-border">
                        ...
                        {c.keyMessaging?.length > 0 && (
                          <div className="mt-2"><div className="text-xs text-muted-foreground mb-1">Key messaging</div>
                            <ul className="list-disc pl-5 text-xs text-muted-foreground space-y-1">{c.keyMessaging.slice(0, 4).map((m, i) => <li key={i}>{m}</li>)}</ul>
                          </div>
                        )}
                      </td>
                      <td className="p-3 border-b border-border text-muted-foreground">{c.positioning}</td>
                      <td className="p-3 border-b border-border text-muted-foreground">{c.pricing}</td>
                      <td className="p-3 border-b border-border"><ul className="list-disc pl-5 text-xs text-muted-foreground space-y-1">{c.strengths?.slice(0, 5).map((s, i) => <li key={i}>{s}</li>)}</ul></td>
                      <td className="p-3 border-b border-border"><ul className="list-disc pl-5 text-xs text-muted-foreground space-y-1">{c.weaknesses?.slice(0, 5).map((w, i) => <li key={i}>{w}</li>)}</ul></td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              <div className="bg-card border border-border rounded-2xl p-4">
                <div className="text-sm font-semibold text-foreground mb-2">Gaps</div>
                <ul className="list-disc pl-5 text-sm text-muted-foreground space-y-1">{competitive.gaps?.map((g, i) => <li key={i}>{g}</li>)}</ul>
              </div>
              <div className="bg-card border border-border rounded-2xl p-4">
                <div className="text-sm font-semibold text-foreground mb-2">Opportunities</div>
                <ul className="list-disc pl-5 text-sm text-muted-foreground space-y-1">{competitive.opportunities?.map((o, i) => <li key={i}>{o}</li>)}</ul>
              </div>
              <div className="bg-card border border-border rounded-2xl p-4">
                <div className="text-sm font-semibold text-foreground mb-2">Keyword gaps</div>
                <div className="text-xs text-muted-foreground mb-2">Topics competitors miss or under-emphasise</div>
