    ...
    <div className="space-y-4 animate-pulse">
      <div className="bg-muted border border-border rounded-2xl p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {[1, 2].map((i) => (
            <div key={i} className="bg-muted/50 border border-border rounded-xl p-4 space-y-3">
...
function MetricBars({ scores, winner }: { scores: VariantScore[]; winner: number }) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      {scores.map((s, idx) => (
        <div key={idx} className="bg-muted border border-border rounded-2xl p-6">
          <div className="flex items-start justify-between gap-4 mb-3">
            <div className="min-w-0">
              <div className="text-xs text-muted-foreground mb-1">Variant {idx + 1}</div>
              <div className="text-sm text-foreground whitespace-pre-wrap break-words">{s.text}</div>
            </div>
            {winner === idx && (
              ...
              return (
                <div key={m.key}>
                  <div className="flex items-center justify-between text-xs mb-1">
                    ...
                    <span className="text-muted-foreground tabular-nums">{value}/10</span>
                  </div>
          ...
          <div className="mt-4 pt-4 border-t border-border">
            ...
            <div className="mt-4">
              <div className="text-xs text-muted-foreground mb-1">Overall comparison</div>
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-4 min-w-0 mb-2">
          ...
          <div className="min-w-0">
            <h1 className="text-2xl font-bold text-foreground break-words">🏆 Variants: {plan.config.app_name}</h1>
            <p className="text-muted-foreground break-words">Paste 2–5 copy variants and score them side-by-side.</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-muted border border-border rounded-2xl p-6 mb-8">
        <div className="flex items-center justify-between gap-4 mb-4">
          <div>
            <div className="text-sm font-semibold text-foreground">Copy variants</div>
            <div className="text-xs text-muted-foreground">One idea per field. Empty fields are ignored.</div>
          </div>
          <div className="text-xs text-muted-foreground tabular-nums">{filledVariants.length}/5 ready</div>
        </div>
          ...
          {variants.map((v, idx) => (
            <div key={idx} className="bg-muted/50 border border-border rounded-xl p-3">
              <div className="flex items-center justify-between mb-2">
                <div className="text-xs text-muted-foreground">Variant {idx + 1}</div>
                <button
                  onClick={() => {
                    setResult(null);
              ...
              <textarea
                ...
                rows={3}
                className="w-full text-sm bg-transparent text-foreground placeholder:text-muted-foreground outline-none resize-y"
              />
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-wrap gap-3 mt-4">
          <button
            ...
            className={`text-sm px-4 py-2 rounded-lg transition-colors font-medium ${variants.length >= 5
                ...
                : 'bg-muted border border-border text-muted-foreground hover:bg-muted/50'
              }`}
          >
            + Add Variant
          </button>
          ...
          {result && (
            <button
              ...
              className="text-sm px-4 py-2 rounded-lg transition-colors font-medium bg-muted border border-border text-muted-foreground hover:bg-muted/50"
            >
              Clear Results
            </button>
          )}
      ...
      {!scoring && result && result.scores?.length > 0 && (
        <div className="space-y-4">
          <div className="flex items-center justify-between gap-4">
            <div>
              <div className="text-sm font-semibold text-foreground">Scores</div>
              <div className="text-xs text-muted-foreground">Winner chosen by strongest overall score.</div>
            </div>
