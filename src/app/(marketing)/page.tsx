...
function Icon({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex h-10 w-10 items-center justify-center rounded-xl border border-indigo-500/20 bg-indigo-500/10 text-indigo-600 dark:text-indigo-300">
      {children}
    </div>
  );
}
...
export default function LandingPage() {
  ...
  return (
    <div className="w-full">
      ...
      {/* HERO */}
      <section className="relative overflow-hidden rounded-3xl border border-slate-200 bg-white/80 p-6 dark:border-slate-800 dark:bg-[#0d1117] sm:p-10">
        ...
        <div className="relative mx-auto max-w-3xl text-center">
          <div className="inline-flex items-center gap-2 rounded-full border border-indigo-500/20 bg-indigo-500/10 px-3 py-1 text-xs text-indigo-700 dark:text-indigo-200">
            <span className="text-slate-600 dark:text-slate-300">Generate a brief in minutes, not days</span>
          </div>
          ...
          {/* CTA */}
          <div className="mt-7 rounded-2xl border border-slate-200 bg-white/70 p-4 dark:border-slate-700/60 dark:bg-slate-900/60 sm:p-5">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <Input
                ...
                className="h-auto rounded-xl px-4 py-3 focus-visible:border-transparent sm:flex-1"
              />
              <Button
                onClick={handleStart}
            ...
            <div className="mt-4 flex flex-wrap items-center justify-center gap-2 text-xs text-slate-600 dark:text-slate-400">
              <span className="rounded-full border border-slate-300 bg-white/80 px-3 py-1 dark:border-slate-700 dark:bg-slate-900/40">Export to Markdown/PDF</span>
              <span className="rounded-full border border-slate-300 bg-white/80 px-3 py-1 dark:border-slate-700 dark:bg-slate-900/40">Built for founders & marketers</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="how" className="mt-12 scroll-mt-24">
        ...
        <div className="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            ...
            <div
              key={s.step}
              className="rounded-2xl border border-slate-200 bg-white/80 p-6 dark:border-slate-800 dark:bg-slate-900/40"
            >
      ...
      <section id="features" className="mt-12 scroll-mt-24">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white sm:text-3xl">Everything you need to ship marketing faster</h2>
          <p className="mt-2 text-sm text-slate-600 dark:text-slate-400 sm:text-base">
        ...
        <div className="mt-7 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {features.map((f) => (
            <div key={f.title} className="rounded-2xl border border-slate-200 bg-white/70 p-5 dark:border-slate-800 dark:bg-slate-900/30">
      ...
      {/* FOOTER */}
      <footer className="mt-14 border-t border-slate-200 pb-10 pt-8 dark:border-slate-800">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
          ...
          <div className="grid grid-cols-2 sm:flex gap-3 sm:gap-6 text-sm">
            <a href="/dashboard" className="text-slate-600 transition-colors hover:text-slate-900 dark:text-slate-400 dark:hover:text-white">
              Dashboard
            </a>
